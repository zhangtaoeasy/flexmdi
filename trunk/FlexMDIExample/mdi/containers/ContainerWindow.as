package mdi.containers
{
	...
	public class ContainerWindow extends Panel
	{
		...
		public function ContainerWindow():void
		{
			super();
			isResizing = false;
		...
		{
			titleBarDoubleClickHandler(event)
			/*
		...
		public function titleBarMoveHandler(event:MouseEvent):void 
		{
			...
			{
				systemManager.addEventListener(MouseEvent.MOUSE_UP, titleBarDragDropHandler);
		...
		{
			systemManager.removeEventListener(MouseEvent.MOUSE_MOVE, resizeMoveHandler);
		...
		protected function getCursorStyle(x:int, y:int, isTitleBar:Boolean):int
		{
			...
			override protected function getHeaderHeight():Number
			{
				...
				if (isNaN(headerHeight))
					headerHeight = measureHeaderText().height + ContainerWindow.HEADER_PADDING;
			...
			protected function resizeMoveListener(event:MouseEvent):void
			{
				...
				if (event.localY > getHeaderHeight() && event.target is ContainerWindow)  
				{
			...
			private function sizeWidth(event:MouseEvent):void
			{
				...
				if (tmp >= minSizeWidth && parent.mouseX < parent.width && parent.mouseX > 0)   
				{
					this.width = tmp;
				}				
			}
			...
			private function sizeHeight(event:MouseEvent):void
			{
				...
				if (tmp >= minSizeHeight && parent.mouseY < parent.height && parent.mouseY > 0)   
				{
					this.height = tmp;
				}
			}
